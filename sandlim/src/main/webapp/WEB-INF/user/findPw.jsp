...
<div class="container">
	    ...
	    <div class="form-group">
		    <div class="label-and-error">
		        ...
		        <span id="error-message" class="error-text hidden">올바른 이름을 입력하세요.</span>
		    </div>
		...
		<div class="form-group">
		    <div class="label-and-error">
		        ...
		        <span id="error-message2" class="error-text hidden">이메일을 올바른 형식으로 작성해주세요.</span>
		    </div>
...
<script>
	    ...
	    if (email === '') {
		    ...
		    $.ajax({
		        ...
		        success : function(data) {
		            ...
		            resetRemTime();
		            verCode = data;
		            document.getElementById('timer2').textContent = "5:00";
	...
	function startTimer() {
        ...
        countdownId = setInterval(function() {
            ...
            document.getElementById('timer2').textContent = minutes + ':' + (seconds < 10 ? '0' : '') + seconds;
            if (remainingTime <= 0) {
    ...
    // 유저비밀번호 페이지 이동
    function requestUserPw() {
    	
    	if(chkEmail){
	        var userName = document.getElementById('u_name').value;
	        var userEmail = document.getElementById('u_email').value;
	        var userId = document.getElementById('u_id').value;
	        // 서버에 비밀번호 페이지로 이동. POST방식으로 이동
           	...
           	$.ajax({
   	            ...
   	            success : function(data) {
   	            	...
   	            	if(userName == "" || data == ""){
   	            		...
   	            		alert('아이디와 회원정보가 일치하지 않습니다.');
   	            		return;
   	            	}
   	            	
   	            	// 소셜계정일경우 id찾기 페이지에서 사용했던 페이지로 보내줍니다.(소셜 계정임을 알려주는 페이지)
   	            	// 소셜계정이 아닌 경우에만 비밀번호 재설정 페이지로 보내줍니다.
   	            	if(msg !== ""){
   	            	    $.ajax({
   	            	        url: 'findIdResult.do',
   	            	        type: 'POST',
